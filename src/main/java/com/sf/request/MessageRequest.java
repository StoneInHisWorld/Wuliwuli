	...
	public @ResponseBody void web3(String message, HttpServletRequest request, HttpServletResponse response,
		...
		if (userName != null) {
			...
			if (message.length() < 150) {
				...
				// new 一个用户实体
				messageEntity messageAgent = new messageEntity();
				messageAgent.setMessage(message);
				messageAgent.setMessageID(messageID);
				messageAgent.setMessageTime(dataTime);
				messageAgent.setMessageuserID(userID);
				messageAgent.setMessageuserName(userName);
				messageAgent.setMessagevideoID(shipingID);
				boolean bl = messageServiceImpl.message(messageAgent);
				if (bl) {
					// 留言成功
